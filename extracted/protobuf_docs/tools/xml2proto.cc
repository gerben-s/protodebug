...
FieldRef FindField(Message* base, const string& name) {
  ...
  const Reflection* reflection = base->GetReflection();
  ...
  if (result.field != NULL &&
    ...
    result.message = reflection->AddMessage(result.message, result.field);
  ...
  while (true) {
    ...
    result.message = reflection->MutableMessage(result.message, result.field);
    descriptor = result.message->GetDescriptor();
    reflection = result.message->GetReflection();
  }
}
...
bool AddText(const string& text, int node_type, Message* message) {
  ...
  if (target.field == NULL) {
    ...
    if (target.field->type() == FieldDescriptor::TYPE_STRING) {
      const Reflection* reflection = target.message->GetReflection();
  ...
  while (true) {
    ...
    switch (node_type) {
      case XML_NODE_ELEMENT: {
        ...
        const Reflection* target_reflection = target.message->GetReflection();
        ...
        if (target.field->cpp_type() == FieldDescriptor::CPPTYPE_MESSAGE) {
          ...
          if (target.field->label() == FieldDescriptor::LABEL_REPEATED) {
            ...
            child = target_reflection->MutableMessage(
              target.message, target.field);
          }
          if (!ReadAttributes(reader, child)) {
            cerr << "Failed to read field: "
                 << target.field->full_name() << endl;
            return false;
          }
          if (!ReadChildren(reader, child)) {
            cerr << "Failed to read field: "
                 << target.field->full_name() << endl;
            return false;
          }
          ...
          free(content);
          if (!ParseSimpleField(target.message, target_reflection,
                                target.field, value)) {
            return false;
          }
          SkipChildren(reader);
